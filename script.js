...
function enviarPedido(infoInicial) {
  ...
  // Só abre o WhatsApp se NÃO estiver em modo teste
  if (!modoTeste) {
    window.open(url, `_blank`);
  } else {
    // Em modo teste, mostra um alerta com os dados do pedido
    Swal.fire({
      icon: "success",
      title: "Pedido Confirmado (Modo Teste)",
      html: `
        <p><strong>${infoInicial}</strong></p>
        <p><strong>Forma de Pagamento:</strong> ${formaPagamento}</p>
        <p><strong>Total:</strong> R$ ${total.toFixed(2).replace(".", ",")}</p>
        <p style="color: #888; font-size: 0.9rem;">Em produção, o WhatsApp seria aberto aqui.</p>
      `,
      confirmButtonText: "OK",
